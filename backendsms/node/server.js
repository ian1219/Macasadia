...
/**
 * Sends a message to api server
 * @param {string} message   
 */
function sendmessagetoresidents(message){
	var headers = {
	...
	(async () => {
	  try {
		const response = await fetch('http://127.0.0.1/macasadia/api/uni_handler.php?type=send_sms_all&message='+message,{ method: 'GET', headers: headers})
...
function messageClients(message) {
	...
	console.log(message);
	const jsonn = JSON.parse(message);
	sendmessagetoresidents(jsonn.message);	
}
